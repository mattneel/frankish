...
extern "C" fn capture_scm_display_str(s: *const u8) {
    let text = unsafe {
        let len = *(s as *const u64) as usize;
        String::from_utf8_lossy(std::slice::from_raw_parts(s.add(8), len)).into_owned()
    };
    CAPTURE.with(|buffer| buffer.borrow_mut().push_str(&text));
}

// The registry check for the capture layer (M17, D-062): typed
...
fn builtin_for(name: &str) -> Option<frk_interp::Builtin> {
    Some(match name {
        ...
        "frk_rt_scm_display_str" => Box::new(|arguments, output| {
...
fn jit_symbol(name: &str) -> Option<*mut ()> {
    Some(match name {
        ...
        "frk_rt_ctl_wind_save" => frk_rt::frk_rt_ctl_wind_save as *mut (),
        "frk_rt_ctl_wind_merge" => frk_rt::frk_rt_ctl_wind_merge as *mut (),
        ...
        "frk_rt_scm_display_str" => capture_scm_display_str as *mut (),
        "frk_rt_ctl_pack_head" => frk_rt::frk_rt_ctl_pack_head as *mut (),
        _ => return None,
    })
}
